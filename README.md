...
## Reality Debugger · Internet Health

The extension is a normal native layer plus the `debug_world` action. Public IODA queries fetch recent country anomaly detections directly over CORS; production does not depend on a Vite proxy. Country outlines are geographic context, not measured nationwide outage coverage. Recovery, affected ASNs and causes remain unknown unless a source establishes them.

`npm run build` produces `dist`; `vercel.json` defines the Vite deployment. Existing optional providers and voice retain their own credential/service requirements. Source outages are reported explicitly and never replaced with invented observations. Run the focused `src/reality/*.test.mjs` and Internet Health layer tests, then the native browser QA before claiming acceptance.
